...
export default function LectureSolutionVideosPage() {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      if (error) {
        console.error("Fetch solution videos error:", error);
        setVideos([]);
      } else {
        setVideos(data || []);
      }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-slate-400">
                  {lectureTitle || "اختر فيديو الحل الذي تريد مشاهدته"}
                </p>
              </div>
            </div>
          </div>
        ...
        {videos.length === 0 ? (
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-10 text-center">
            ...
            <p className="text-slate-400 text-lg">
              لا توجد فيديوهات حل متاحة حالياً
            </p>
          </div>
        ) : (
          <div className="space-y-5">
            {videos.map((video, index) => (
              <Link
                key={video.id}
                href={`/lectures/${lectureId}/solution-videos/${video.id}`}
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h2 className="text-lg md:text-xl font-bold text-white truncate">
                        {video.title || `فيديو الحل ${index + 1}`}
                      </h2>
